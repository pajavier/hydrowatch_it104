      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-white/10 bg-[#111A38] p-4">
          ...
          <NumericInput
            ...
            step={0.1}
            onChange={(v) => onSave({ ...settings, alertSensitivity: v })}
          />
          <NumericInput
            label="Auto-refresh ms"
            value={settings.refreshIntervalMs}
            step={500}
            onChange={(v) => onSave({ ...settings, refreshIntervalMs: v })}
          />
          <NumericInput
            label="Prediction aggressiveness"
            value={settings.predictionAggressiveness}
            step={0.1}
            onChange={(v) => onSave({ ...settings, predictionAggressiveness: v })}
          />
